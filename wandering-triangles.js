...
wanderingTriangles.draw = function (instance) {
  ...
  if (instance.enabled) {
    var entityCount =
      (canvas.width / settings.triangleSize) * settings.triangleDensity;
...
wanderingTriangles.pick = function (odds) {
  var total = 0.0;
  for (var i = 0; i < odds.length; i++) total += odds[i];
...
wanderingTriangles.updateTriangle = function (instance, triangle) {
  ...
  if (triangle.type === this.constants.UP)
    triangle = this.nextTriangleFromUp(
      triangle,
      settings.triangleSize,
      settings.directionBias
    );
  else if (triangle.type === this.constants.DOWN)
    triangle = this.nextTriangleFromDown(
      triangle,
      settings.triangleSize,
      settings.directionBias
    );
  else if (triangle.type === this.constants.LEFT)
    triangle = this.nextTriangleFromLeft(
      triangle,
      settings.triangleSize,
      settings.directionBias
    );
  else
    triangle = this.nextTriangleFromRight(
      triangle,
      settings.triangleSize,
      settings.directionBias
    );
  ...
  if (Math.random() > settings.chanceOfJump && !offscreen) return triangle;
  ...
  else triangle.colour = settings.primaryColour;

  return triangle;
};

wanderingTriangles.nextTriangleFromUp = function (
  triangle,
  triangleSize,
  directionBias
) {
...
wanderingTriangles.nextTriangleFromDown = function (
  triangle,
  triangleSize,
  directionBias
) {
...
wanderingTriangles.nextTriangleFromLeft = function (
  triangle,
  triangleSize,
  directionBias
) {
...
wanderingTriangles.nextTriangleFromRight = function (
  triangle,
  triangleSize,
  directionBias
) {
...
wanderingTriangles.drawTriangle = function (instance, triangle) {
  ...
  context.closePath();
  if (Math.random() > settings.chanceOfFill) context.stroke();
  else context.fill();
};
